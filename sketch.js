...
function draw() {
  ...
  //when player touch the enemy , destroy it and increase
  ...
  if (player.isTouching(enemy4) && score === 600) {
    enemy4.destroy();
    //score = score + 200;
  ...
  //when player the key but all the enemies are not destroyed 
  if (player.isTouching(enemy4) && score < 600) {
  ...
  //when temp is 1 show you win and restart
  if (temp === 1) {
    text("YOU WIN!", 450, 275);
    text("Press K to restart", 450, 365);
  ...
  if (player.x > 1330 || player.x < 10 || player.y > 720 || player.y < 0) {
    player.destroy();
